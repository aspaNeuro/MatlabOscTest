classdef Osc < handle
  ...
  methods (Static)
    ...
    function message = parse(oscMessage)
      ...
      if length(strList) > 1
        t = strsplit(strList{2},'\0');
        types = t{1};
        adress = strList{1};
        offset = length(adress);
        typesPaddedLenght = ceil((length(types) + length(',') + 1)/4)*4;% +1 from '\0'
        offset = offset+typesPaddedLenght+1;
        for count = 1:length(types)
          code = types(count);
          if code == 'i'
            value = typecast(uint8(fliplr(oscMessage(offset:offset+3))), 'uint32');
            offset = offset + 4;
          elseif code == 'h'
            value = typecast(uint8(fliplr(oscMessage(offset:offset+7))), 'uint64');
            offset = offset + 8;
          elseif code == 'f'
